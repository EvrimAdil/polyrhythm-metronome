...
class AudioEngine {
  ...
  synthesizeClick(layer, accent, triggerTime) {
    ...
    const baseFreq = layer.synthFreq * (isDownbeat ? 1.6 : 1.0);
    const baseGain = layer.volume * (isDownbeat ? 1.25 : 0.85);
    const decayDuration = isDownbeat ? 0.025 : 0.015;

    const osc = this.audioCtx.createOscillator();
    const gainNode = this.audioCtx.createGain();

    osc.type = 'sine';
    osc.frequency.setValueAtTime(baseFreq * 1.5, triggerTime);
    osc.frequency.exponentialRampToValueAtTime(baseFreq, triggerTime + 0.006);

    gainNode.gain.setValueAtTime(baseGain, triggerTime);
    gainNode.gain.exponentialRampToValueAtTime(0.0001, triggerTime + decayDuration);
    ...
    osc.stop(triggerTime + decayDuration + 0.01);
  }
}
...
class RotaryJogWheel {
  ...
  bindEvents() {
    ...
    const onMove = (clientX, clientY) => {
      ...
      const bpmDelta = (delta / (Math.PI * 2)) * 30;
      if (Math.abs(bpmDelta) >= 0.25) {
        this.onBpmDelta(Math.round(bpmDelta));
      }
    };
    ...
    this.el.addEventListener('pointermove', (e) => onMove(e.clientX, e.clientY));
